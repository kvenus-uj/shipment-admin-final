...
export class InventoryComponent implements OnInit {
  ...
  get getFilteredPageData() {
    var filteredData = this.tableData.filter(inbound =>
      ...
      inbound.customer.toLowerCase().indexOf(this.filterName.toLowerCase()) > -1 &&
  ...
  export() {
    var content = "SKU\tCUSTOMER\tPRODUCT NAME\tAVAILABLE STOCK\tRESERVED STOCK\n";
    this.tableData.forEach(function (row) {
      content += "" + row.sku + "\t" + row.customer + "\t" + row.productName + "\t" + row.availableStock + "\t" + row.reservedStock + "\n";
    });
