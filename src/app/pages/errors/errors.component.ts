...
export class ErrorsComponent implements OnInit {
  ...
  get getFilteredPageData() {
    var filteredData = this.tableData.filter(inbound =>
      ...
      inbound.customer.indexOf(this.filterName) > -1 &&
  ...
  export() {
    var content = "ERROR TYPE\tORDER REFERENCE\tCUSTOMER\tTRACKING ID\tCREATION DATE\tRECIPIENT\tCOUNTRY\tDELIVERY MODE\n";
    this.getFilteredData.filter(a => a.checked).forEach(function (row) {
      content += "" + row.errorType + "\t" + row.orderReference + "\t" + row.customer + "\t" + row.trackingID + "\t" + row.creationDate + "\t" + row.recipient + "\t" + row.country + "\t" + row.deliveryMode + "\n";
    });
